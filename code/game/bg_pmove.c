...
if ( ! (pm->ps->aiChar))  // RealRTCW weapon weight does not affect AI now
	{ 
		...
		if ( ( pm->ps->weapon == WP_LUGER ) || ( pm->ps->weapon == WP_COLT ) || ( pm->ps->weapon == WP_AKIMBO ) || ( pm->ps->weapon == WP_SILENCER ) || ( pm->ps->weapon == WP_DYNAMITE ) || ( pm->ps->weapon == WP_POISON_GAS ) || ( pm->ps->weapon == WP_GRENADE_LAUNCHER ) || ( pm->ps->weapon == WP_SMOKE_GRENADE ) || ( pm->ps->weapon == WP_GRENADE_PINEAPPLE ) || ( pm->ps->weapon == WP_REVOLVER ) ) {
			scale *= 0.95; 
		}
